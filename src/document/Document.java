...
public abstract class Document {
	...
	// in the text. You should write this and use it in your
	// BasicDocument class.
	...
	protected int countSyllables() {
		...
		int count = 0, totalCount = 0, vowelStreak = 0;
		List<String> words = getTokens("[a-zA-Z]+");
		//System.out.println();
		for (String word : words) {
			char[] wordCopy = word.toLowerCase().toCharArray();
			count = 0;
			vowelStreak = 0;
			int n = word.length();
			for (int i = 0; i < n; i++) {
				char c = wordCopy[i];
				if (i == n - 1 && c == 'e' && vowelStreak == 0 && count != 0) {
					// lone 'e' is present and there are some other syllables
				} 
				else if (c == 'a' | c == 'e' | c == 'i' | c == 'o' | c == 'u' | c == 'y') {
					if (i == n - 1) {
						count++;
						totalCount++;
						//System.out.printf("|" + totalCount + " ->" + c);
					} else {
						vowelStreak++;
						//System.out.printf("" + c);
					}
				} else if (vowelStreak != 0) {
					vowelStreak = 0;
					count++;
					totalCount++;
	...
	public double getFleschScore() {
		...
		if(getNumSentences() != 0)
			flesch = 206.835  - 1.015 * ((double)getNumWords()/getNumSentences()) - 84.6 * ((double)getNumSyllables()/getNumWords());
		return flesch;
	}
}
